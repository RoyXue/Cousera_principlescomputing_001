#The homework's question is just too long
#So i didnt read that
#I just write some code to make it can generate the same answer with sample
...
def resources_vs_time(upgrade_cost_increment, num_upgrade):
    ...
    s = []
    time = 0
    for i in range(0, num_upgrade):
        if i == 0:
            total += a
            time += a
            s.append([time, total])
            continue
        a += upgrade_cost_increment
        avg = a/(i+1)
        time += avg
        total += a
        s.append([time, total])
...
def test():
    ...
    data2 = resources_vs_time(0.5, 50)
    data3 = resources_vs_time(1.0, 50)
    data4 = resources_vs_time(2.0, 50)
